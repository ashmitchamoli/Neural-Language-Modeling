...
class LstmLanguageModel(BaseLanguageModel):
	def __init__(self, trainDataset : LstmLanguageModelDataset, 
		...
		super().__init__(trainDataset, pretrainedEmbeddings, fineTunedPretrained)
	...
	def forward(self, x : torch.Tensor) -> torch.Tensor:
		...
		"""
		x = self._getEmbeddings_(x) # (batchSize, maxSentenceLength, pretrainedEmbeddingSize)
		x, _ = self.lstm(x) # (batchSize, maxSentenceLength, hiddenEmbeddingSize * (bidirectional + 1))
		x = self.linear(x) # (batchSize, maxSentenceLength, vocabSize)
		x = self.softmax(x)
